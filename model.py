...
def vis(args):
    ...
    #    image_path = args.image_path  # 绝对路径
    img = args.image
    ...
    with torch.no_grad():
        ...
        #     f.write(image_path.split("/")[-static] + " " + str(num))
        vis_img = pred_map
        # # normalize density map values from 0 to static, then map it to 0-255.
        vis_img = (vis_img - vis_img.min()) / (vis_img.max() - vis_img.min() + 1e-5)
        vis_img = (vis_img * 255).astype(np.uint8)
        vis_img = cv2.applyColorMap(vis_img, cv2.COLORMAP_JET)
        save_path = r"E:\桌面\temp"
